...
const EditRental = () => {
  ...
  const { data, loading, error } = useQuery(GET_RENTALS_BY_PK, {
    variables: {
      id: rentalId,
    },
    onCompleted(data) {
      reset({
        image: data?.rentals_by_pk?.image,
        name: data?.rentals_by_pk?.name,
        description: data?.rentals_by_pk?.description,
        location: data?.rentals_by_pk?.location,
        country_id: data?.rentals_by_pk?.country,
        city_id: data?.rentals_by_pk?.city,
        yacht_id: data?.rentals_by_pk?.yacht,
      });
    },
  });
  ...
  const submit = (values) => {
    ...
    callApi(userToken, "multipart/form-data")
      .post(`/owners/update-rentals/${rentalId}`, formData)
      .then((data) => {
        if (data.status == 200 || data.status == 201) {
          fireAlert("Rental Updated Successfully");
        } else {
          throw data;
        }
      })
    ...
    >
      <Stack direction="row" gap="16px">
        <Stack flex="1" gap="16px">
          <UploadImage
            defaultValue={data?.rentals_by_pk?.image}
