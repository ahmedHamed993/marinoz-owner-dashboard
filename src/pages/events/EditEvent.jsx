...
const EditEvent = () => {
  ...
  const { data, loading, error } = useQuery(GET_EVENTS_BY_PK, {
    variables: {
      id: eventId,
    },
    onCompleted(data) {
      reset({
        image: data?.events_by_pk?.image,
        name: data?.events_by_pk?.name,
        description: data?.events_by_pk?.description,
        location: data?.events_by_pk?.location,
        country_id: data?.events_by_pk?.country,
        city_id: data?.events_by_pk?.city,
        yacht_id: data?.events_by_pk?.yacht,
      });
    },
  });
  ...
  const submit = (values) => {
    ...
    callApi(userToken, "multipart/form-data")
      .post(`/owners/update-events/${eventId}`, formData)
      .then((data) => {
        if (data.status == 200 || data.status == 201) {
          fireAlert("Event Updated Successfully");
        } else {
          throw data;
        }
      })
    ...
    >
      <Stack direction="row" gap="16px">
        <Stack flex="1" gap="16px">
          <UploadImage
            defaultValue={data?.events_by_pk?.image}
