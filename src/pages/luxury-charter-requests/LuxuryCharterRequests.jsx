...
import {GET_LUXURY_CHARTER_REQUESTS} from "./queries";
import { Box, Divider, Stack, Typography } from '@mui/material';
const LuxuryCharterRequests = () => {
    const {data,loading,error} = useQuery(GET_LUXURY_CHARTER_REQUESTS);
    return loading? <p>loading...</p> : (
        <Stack>
            <Typography variant="h5">Luxury Charter Requests</Typography>
            <Box sx={{
                display:"grid",
                gridTemplateColumns:"repeat(3,1fr)",
            }}>
                {data?.luxury_charter_requests?.map((event)=>{
                   return (
                    <Stack
                    divider={<Divider />}
                    sx={{
...
export default LuxuryCharterRequests
