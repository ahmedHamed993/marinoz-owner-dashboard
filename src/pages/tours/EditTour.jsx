...
const EditTour = () => {
  ...
  const { data, loading, error } = useQuery(GET_TOURS_BY_PK, {
    variables: {
      id: tourId,
    },
    onCompleted(data) {
      reset({
        image: data?.tours_by_pk?.image,
        name: data?.tours_by_pk?.name,
        description: data?.tours_by_pk?.description,
        location: data?.tours_by_pk?.location,
        country_id: data?.tours_by_pk?.country,
        city_id: data?.tours_by_pk?.city,
        yacht_id: data?.tours_by_pk?.yacht,
      });
    },
  });
  ...
  const submit = (values) => {
    ...
    callApi(userToken, "multipart/form-data")
      .post(`/owners/update-tours/${tourId}`, formData)
      .then((data) => {
    ...
    >
      <Stack direction="row" gap="16px">
        <Stack flex="1" gap="16px">
          <UploadImage
            defaultValue={data?.tours_by_pk?.image}
