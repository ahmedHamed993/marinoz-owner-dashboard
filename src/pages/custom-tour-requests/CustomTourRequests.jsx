...
import {GET_CUSTOM_TOUR_REQUESTS} from "./queries";
import { Box, Divider, Stack, Typography } from '@mui/material';

const CustomTourRequests = () => {
    const {data,loading,error} = useQuery(GET_CUSTOM_TOUR_REQUESTS);

    return loading? <p>loading...</p> : (
        <Stack>
            <Typography variant="h5">Custom Tour Requests</Typography>
            <Box sx={{
                display:"grid",
                gridTemplateColumns:"repeat(3,1fr)",
            }}>
                {data?.custom_tour_requests?.map((event)=>{
                   return (
                    <Stack
                    divider={<Divider />}
                    sx={{
